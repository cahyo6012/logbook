...
class Logbook {
  ...
  async login() {
    ...
    if('refresh' in (res.headers|| {}) || this.checkLogin(res.body)) console.log('Login Berhasil...')
    else {
  ...
  async getToken(url = this.BASE_URL, withData = false) {
  ...
  async presensi() {
    ...
    const form = await this.getToken(url, true)
  ...
  async isiSak() {
    ...
    form.token = await this.getToken(url + '/form')
